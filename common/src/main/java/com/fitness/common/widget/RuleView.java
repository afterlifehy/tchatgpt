...
//        canvas.drawLine(0, shortLineWidth * .5f, mWidth, 0, mPaint);
        ...
        while (startNum <= rightMaxNum) {
            ...
            if (startNum % (perUnitCount / 2) == 0 && (startNum / (perUnitCount / 2)) % 2 != 0) {
                mPaint.setStrokeWidth(shortLineWidth);
                canvas.drawLine(distance, 0, distance, shortGradationLen * 3 / 2, mPaint);
            } else if (startNum % perUnitCount == 0) {
                // 长刻度：刻度宽度为短刻度的2倍
                ...
                if (currentValue == Float.parseFloat(text)) {
                    mTextPaint.setTypeface(Typeface.createFromAsset(BaseApplication.instance().getAssets(), "fonts/Alibaba-PuHuiTi-Heavy.otf"));
                    mTextPaint.setColor(ContextCompat.getColor(BaseApplication.instance(), com.fitness.base.R.color.color_ff6a93ce));
                    mTextPaint.setTextSize(sp2px(20f));
                    canvas.drawText(text, distance - textWidth * .9f, longGradationLen + gradationNumberGap + textSize, mTextPaint);
                } else {
